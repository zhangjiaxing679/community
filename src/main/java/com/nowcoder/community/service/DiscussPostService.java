...
public class DiscussPostService {
    ...
    //帖子总数缓存
    private LoadingCache<Integer,Integer> postRoesCache;
        ...
        postListCache = Caffeine.newBuilder()
                ...
                .build(new CacheLoader<String, List<DiscussPost>>() {
                    ...
                    public @Nullable List<DiscussPost> load(String key) throws Exception {
                        ...
                        int limie= Integer.valueOf(params[1]);

                        // 二级缓存，Redis->mysql
                        logger.info("load post list from DB.");
                        return discussPostMapper.selectDiscussPosts(0,offset,limie,1);
                    }
                });

        //初始化帖子总数缓存
        postRoesCache = Caffeine.newBuilder()
                .maximumSize(maxSize)
                .expireAfterWrite(expireSeconds,TimeUnit.SECONDS)
    ...
    {
        if (userId == 0) {
            return postRoesCache.get(userId);
        }
